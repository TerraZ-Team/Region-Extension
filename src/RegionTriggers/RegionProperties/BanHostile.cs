...
namespace RegionExtension.RegionTriggers.RegionProperties
{
    public class BanHostile : IRegionProperty
    {
        ...
        private void OnProjectileUpdate(ProjectileAiUpdateEventArgs args)
        {
            ...
            if (!proj.hostile)
                return;
            if (IsInAnyProtectedRegion((int)(proj.position.X / 16), (int)(proj.position.Y / 16)))
                proj.Kill();
        }
        ...
        private void OnPostUpdate(EventArgs args)
        {
            ...
            {
                var boss = _ignoreNpc[i];
                if (!boss.Ignore)
                    continue;
                var npc = Main.npc[boss.whoAmI];
                var x = (int)(npc.position.X / 16);
                var y = (int)(npc.position.Y / 16);
                var lastY = (int)(boss.LastPosition.Y / 16);
                var lastX = (int)(boss.LastPosition.X / 16);
                var resX = false;
                var resY = false;
                if (IsInAnyProtectedRegion(x, lastY))
                {
                    npc.position.X = boss.LastPosition.X;
                    npc.velocity.X = 0;
                    resX = true;
                }
                if (IsInAnyProtectedRegion(lastX, y))
                {
                ...
                {
                    var _pushDistance = 1;
                    if (!TryGetHighestPriorityRegionArea(x, y, out var regArea))
                        continue;
                ...
                {
                    //skeletron prime fix
                    if (npc.type == NPCID.SkeletronPrime || npc.ai[1] == 32)
                        npc.ai[1] = 0;
                    if (lastSend[npc.whoAmI].AddMilliseconds(100) < DateTime.Now)
                    {
                        NetMessage.SendData((int)PacketTypes.NpcUpdate, number: npc.whoAmI);
        ...
        private void OnAIUpdate(NpcAiUpdateEventArgs args)
        {
            ...
            if (!IsInAnyProtectedRegion(x, y))
                return;
            Main.npc[args.Npc.whoAmI].active = false;
            NetMessage.SendData((int)PacketTypes.NpcStrike, number: args.Npc.whoAmI, number2: -1);
            short pid = -1;
            ...
            PacketConstructor.SendPacket(-1, new ProjectileDestroyPacket(pid, byte.MaxValue));
        }

        private bool IsInAnyProtectedRegion(int x, int y)
        {
            foreach (var pair in _regions)
                if (pair.Key.InArea(x, y))
                    return true;
            return false;
        }

        private bool TryGetHighestPriorityRegionArea(int x, int y, out Rectangle area)
        {
            Region best = null;
            foreach (var pair in _regions)
            {
